...
// ✅ GET: Get progress of a specific examinee
router.get('/progress/:examineeId', async (req, res) => {
  try {
    const { examineeId } = req.params;

    // check valid ID
    if (!mongoose.Types.ObjectId.isValid(examineeId)) {
      return res.status(400).json({ error: `Invalid examinee ID: ${examineeId} `});
    }

    // fetch all attempts
    const attempts = await ExamAttempted.find({ examineeId })
    ...
    const passedExams = attempts.filter(a => a.status === 'Passed').length;
    const failedExams = attempts.filter(a => a.status === 'Failed').length;
    const avgScore = attempts.reduce((sum, a) => sum + a.score, 0) / totalExams;

    const progressReport = {
      examinee: {
        name: attempts[0].examineeId?.name || "N/A",
        email: attempts[0].examineeId?.email || "N/A",
      },
      totalExams,
      passedExams,
      failedExams,
